...
m = Map("ttyS1", translate("<abbr title=\"Light Emitting Diode\">ttyS1</abbr> Configuration"), translate("Customizes the behaviour of the device <abbr title=\"Light Emitting Diode\">ttyS1</abbr>s if possible."))

local sysfs_path = "/usr/local/gateway/config/gateway_config.json"
local ttyS1 = {}
...
if fs.access(sysfs_path) then
	ttyS1 = util.consume((fs.dir(sysfs_path)))
end
